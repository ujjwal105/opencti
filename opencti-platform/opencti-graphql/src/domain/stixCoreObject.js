...
export const stixCoreObjectExportAsk = async (context, user, stixCoreObjectId, args) => {
  const { format, exportType = null, contentMaxMarkings = null, fileMaxMarkings = null } = args;
  const entity = await storeLoadById(context, user, stixCoreObjectId, ABSTRACT_STIX_CORE_OBJECT);
  const works = await askEntityExport(context, user, format, entity, exportType, contentMaxMarkings, fileMaxMarkings);
  return works.map((w) => workToExportFile(w));
};
...
export const stixCoreObjectExportPush = async (context, user, entityId, args) => {
  ...
  const { upload: up } = await upload(context, user, path, args.file, { entity: previous, content_max_markings: args.content_max_markings, file_max_markings: args.file_max_markings });
