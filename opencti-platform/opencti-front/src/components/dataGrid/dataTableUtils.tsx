...
const defaultColumns: DataTableProps['dataColumns'] = {
  ...
  entity_type: {
    ...
    flexSize: 10,
    isSortable: false,
    render: (data, { t_i18n, storageHelpers }) => {
      const classes = useStyles();
      return (
        <>
          ...
          <Chip
            classes={{ root: classes.chipInList }}
            onClick={() => storageHelpers.handleAddFilter('entity_type', data.entity_type)}
            style={{
  ...
  note_types: {
    ...
    flexSize: 10,
    isSortable: true,
    render: ({ note_types }, { storageHelpers, t_i18n }) => {
      const classes = useStyles();
      return (
        <Chip
          classes={{ root: classes.chipInList }}
          onClick={() => storageHelpers.handleAddFilter('report_types', note_types?.at(0))}
          color="primary"
          variant="outlined"
  ...
  report_types: {
    ...
    flexSize: 10,
    isSortable: true,
    render: ({ report_types }, { storageHelpers, t_i18n }) => {
      const classes = useStyles();
      return (
        <Chip
          classes={{ root: classes.chipInList }}
          onClick={() => storageHelpers.handleAddFilter('report_types', report_types?.at(0))}
          color="primary"
          variant="outlined"
  ...
  x_opencti_workflow_id: {
    ...
    flexSize: 8,
    isSortable: true,
    render: ({ status, workflowEnabled }, { storageHelpers }) => (
      <ItemStatus
        onClick={() => storageHelpers.handleAddFilter('workflow_id', status.template.id)}
        status={status}
        variant="inList"
